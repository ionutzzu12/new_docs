...
slug: osi-pi-historian-integration-guide
docTags: 
...
- A special character must be followed by either itself or one of the supported characters. Refer to :Link[OSI PI Historian]{label="OSI PI Historian" spaceId="7QvCN5MYpwobQXnPvWc7V" docId="JJiyDJGKrHRVo5OCfTcJ3" version="v2" docAnchorId loadingMethod="dynamic" overridedLabel} to get the list of symbols with their definition. Any other condition will result in an error:
  **Example 1:** `$D.$N$%` -> error
  **Example 2:** `$D.$N$` -> error
  **Example 3:** `$D.$N$#` -> error
