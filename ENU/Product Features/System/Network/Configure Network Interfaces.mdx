...
slug: product-features/system/network/configure-network-interfaces
...
Maximum Transmission Unit (MTU) represents the largest packet size that a network can send without splitting packets. Set the correct MTU size to prevent fragmentation and dropped packets and to keep connections stable. For example, a common MTU size for Ethernet is 1500 bytes.

Header data is added to each packet when you use a tunnel, like a VPN or IPsec. Packets can get split or dropped when the total packet size, including the tunnel overhead, exceeds the network's MTU. This can cause slow connections or data loss. Lower the MTU on both sides of the tunnel to ensure that packets stay under the size limit and move through the network without being split.
...
4. Click **Update&#x20;**&#x66;or an interface. Update the details for the interface.&#x20;
   ...
   - **MTU**: Specify the largest packet size that the network can send without splitting packets.
5. Click **Update**.&#x20;
6. Reboot your device to apply any changes to network configuration.
