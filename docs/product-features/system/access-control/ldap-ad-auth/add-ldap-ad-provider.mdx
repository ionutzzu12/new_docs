...
slug: product-features/system/access-control/ldap-ad-auth/add-ldap-ad-provider
...
Review the :Link[Set Up LDAP Using an OpenLDAP Container]{label="Set Up LDAP Using an OpenLDAP Container" spaceId="JWQMp_kvm0WsA_Fpjt1v_" docId="gmFG-6WjseBVxq8iHVOHw" version="v2" docAnchorId loadingMethod="dynamic" overridedLabel} how-to guide.&#x20;
...
An LDAP Bind DN supplies the user and the user location in the LDAP directory tree. The LDAP client configuration file contains this information. See [Find LDAP Distinguished Names (DN)](find-ldap-distinguished-names-dn.mdx) for more information.
...
1. Configure the *Connection&#x20;*&#x73;ettings.&#x20;
   ...
   - **Use TLS**: Select the checkbox to enable TLS authentication. When TLS is not enabled, Litmus Edge expects to find a configured Custom Certificate. See :Link[Add a Custom CA Certificate]{docId="ifuyfnv42ngJAQxvTsGhk" label="Add a Custom CA Certificate" spaceId="JWQMp_kvm0WsA_Fpjt1v_" version="v2" docAnchorId loadingMethod="dynamic" overridedLabel}.&#x20;
   - **TLS Root CA**: If you select TLS authentication, paste the root SSL/TLS certificate or click **Upload&#x20;**&#x61;nd load the file.&#x20;
   - **Bind DN**: Enter the bind DN identifier. The bind DN identifies the user and the location of the user in the LDAP directory tree. See [Find LDAP Distinguished Names (DN)](find-ldap-distinguished-names-dn.mdx).
...
1. Configure the *User&#x20;*&#x73;ettings.&#x20;
   - **User Search Base DN**: Enter a value. This Base DN (Distinguished Name) is the point in the LDAP directory tree that the LDAP service uses to initiate a user search. The Base DN is the latter part of the Bind DN. See [Find LDAP Distinguished Names (DN)](find-ldap-distinguished-names-dn.mdx).
