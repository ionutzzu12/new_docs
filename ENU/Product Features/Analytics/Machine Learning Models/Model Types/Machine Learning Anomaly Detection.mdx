...
slug: product-features/analytics/machine-learning-models/model-types/machine-learning-anomaly-detection
...
<BlockQuote>
  ...
  sns.set(style='whitegrid', palette='muted')# rcParams\['figure.figsize'] = 22, 10csv\_path = '20161003\_085624.csv'
  ...
  df = features
  print(x)# plt.plot(df)
  \# plt.show()train\_size = int(len(df) \* 0.90)
  ...
  test\['close'] = scaler.transform(test\[\['Temperature (C)']])# print(train, test)def create\_dataset(X, y, time\_steps=1):
  Xs, ys = \[], \[]
  ...
  ​
  TIME\_STEPS = 30# reshape to \[samples, time\_steps, n\_features]
  ...
  print(y\_test.shape)
  print(X\_train.shape\[1], X\_train.shape\[2], y\_train\[1])# model = keras.Sequential()
  ...
  \# model.compile(loss='mae', optimizer='adam')verbose, epochs, batch\_size = 0, 2, 128
  ...
  ​
  model.summary()
  X\_train\_pred = model.predict(X\_train)​# model.save("/anomalyModel/")
  ​
